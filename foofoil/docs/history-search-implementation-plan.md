...
本文档定义 foofoil 历史记录搜索功能的完整实施方案，作为后续编码、测试和验收的依据。
...
- 不作为任何 foofoil 内容窗口的 sheet 或 child window。
...
- 无论当前是否存在空白窗口，搜索结果始终在一个新的 foofoil 窗口中打开。
...
```text
~/Library/Application Support/foofoil/history.sqlite3
```
...
- `foofoil.xcodeproj/project.pbxproj`
