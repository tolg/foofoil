...
public class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    public func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleCreateNewFlaminaFromImage(_:)),
    ...
    private func setupMainMenu() {
        ...
        let newWindowItem = NSMenuItem(title: NSLocalizedString("New Flamina", comment: ""), action: #selector(newWindowAction), keyEquivalent: "n")
        ...
        let closeWindowItem = NSMenuItem(title: NSLocalizedString("Close Flamina", comment: ""), action: #selector(closeWindowAction), keyEquivalent: "w")
        ...
        // Capture Image Flamina - 截取图片箔
        let captureImageFlaminaItem = NSMenuItem(
            title: NSLocalizedString("Capture Image Flamina", comment: ""),
            action: #selector(captureImageFlaminaAction),
            keyEquivalent: ""
        )
        captureImageFlaminaItem.withSymbol("camera.viewfinder")
        captureImageFlaminaItem.target = self
        viewMenu.addItem(captureImageFlaminaItem)
            ...
            if let webURL = appState.webURL {
                if webURL.isFileURL {
                    ...
                    guard let imageURL = appState.imageURL else {
                        throw NSError(domain: "FlaminaError", code: 404, userInfo: [NSLocalizedDescriptionKey: "Snapshot image not found"])
                    }
                    if FileManager.default.fileExists(atPath: targetURL.path) {
                        try FileManager.default.removeItem(at: targetURL)
                    }
    ...
    @objc private func captureImageFlaminaAction() {
        guard let appState = activeAppState, appState.webURL != nil else { return }
        NotificationCenter.default.post(
            name: Notification.Name("captureImageFlamina_\(appState.id.uuidString)"),
            object: nil
        )
    }
    ...
    @objc private func handleCreateNewFlaminaFromImage(_ notification: Notification) {
        guard let userInfo = notification.userInfo,
              let id = userInfo["id"] as? UUID,
...
extension AppDelegate: NSMenuItemValidation, NSMenuDelegate {
    ...
    private func updateViewMenu(_ menu: NSMenu) {
        guard let appState = activeAppState else {
            ...
            setMenuItem(withAction: #selector(captureImageFlaminaAction), in: menu, isHidden: true)
        ...
        // 1.6 Capture Image Flamina 菜单项
        setMenuItem(withAction: #selector(captureImageFlaminaAction), in: menu, isHidden: !isWebMode)
