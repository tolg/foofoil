...
nonisolated final class HistoryDatabase {
    ...
    private let queue = DispatchQueue(label: "com.flamina.history.database", qos: .utility)
    private var connection: OpaquePointer?
    let databaseURL: URL

    /// 将旧 Flofoil 支持目录迁移为 Flamina，仅当新目录不存在时执行。
    private static func migrateLegacySupportDirectoryIfNeeded() {
        ...
        let legacy = appSupportURL.appendingPathComponent("Flofoil", isDirectory: true)
        let target = appSupportURL.appendingPathComponent("Flamina", isDirectory: true)
    ...
    init(databaseURL: URL? = nil) throws {
        if let databaseURL {
            ...
            // 若迁移未执行但旧库仍存在（如新目录已存在导致未自动迁移），尝试复制旧数据库
